...
- **2026-05-30**: Fixed same-tab chatbot memory. Root cause was that the browser retained messages only for display while sending the proxy only the latest `question`; the proxy then called Mistral with no prior turns. Updated the client to send recent user/assistant turns and use recent user turns for local retrieval, updated the Worker to forward sanitized recent history to Mistral, and updated the chatbot SOP/schema. Verified with `npm test -- quartz/components/scripts/chatbot.helpers.test.ts`, `npx tsc --noEmit`, `npx wrangler deploy --dry-run`, and `npm run build`.
